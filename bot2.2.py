...
def get_tide(lat, lon):
    ...
    if key in tide_cache:
        if now - tide_cache[key]["time"] < 21600:
    ...
    try:
        ...
        print("TIDE STATUS:", r.status_code)

        if r.status_code != 200:
            print("Errore HTTP maree:", r.text[:200])
            return "N/D", "N/D", "errore"
        ...
        print("Errore maree:", e)

        # ✅ fallback intelligente
        fallback_high = "06:00"
        fallback_low = "12:00"

        return fallback_high, fallback_low, "stimato"
...
def genera_report(lat, lon):
    ...
    # maree
    alta, bassa, fonte_marea = get_tide(lat, lon)
